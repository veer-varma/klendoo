...
describe("payViaIntermezzo", () => {
  it("posts user_id and url to the gateway's x402/fetch route with the X-Klendoo-API-Key header", async () => {
    ...
    expect(fetchImpl).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        method: "POST",
        // Confirmed by Manus (2026-08-19): only X-Klendoo-API-Key — the
        // gateway creates its own short-lived Authorization: Bearer <JWT>
        // to Intermezzo internally, this client never sends one.
        headers: expect.objectContaining({ "X-Klendoo-API-Key": "test-key" }),
